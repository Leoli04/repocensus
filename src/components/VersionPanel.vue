...
const changelog: ChangelogEntry[] = [
  {
    ...
    type: 'release',
    items: [
      '多用户对比升级：由固定「我 vs 1 人」扩展为两大类 —— 「我和他人」（我 + 至多 3 人）与「他人和他人」（2~4 人），每次总人数不超过 4',
      'chips 式输入：输入用户名回车添加、点 × 移除，重复 / 加自己 / 超上限均有即时提示；对比名单存本机，下次打开自动带出',
      '领先高亮推广：每项指标在全部对比侧中取最优值标绿（并列都标），不再是二元胜负',
      '重叠分析分模式：「我和他人」展示我 vs 每人的仓库重叠；「他人和他人」展示两两配对重叠（4 人最多 6 组），均按重叠数降序',
      '口径修正：所有对比侧统一排除 fork 仓库（旧版仅「我」侧排除，两侧口径不一致）',
      '匿名即可用：浏览器端匿名调用 GitHub 公共 API（60 次/小时），一次 4 人对比约 12 个请求；单人 404 自动跳过并提示，限流（403）直接终止',
    ],
  },
  {
    version: 'v1.15.0',

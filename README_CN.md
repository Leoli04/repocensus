...
| **Trending 热榜**（我的仓库） | 我的仓库 Star 增速排行 + 月均 velocity，支持分类筛选 |
| **全球热门榜**（附属工具） | 抓取 GitHub 全站 Trending：今日/本周/本月 × 11 组语言榜，含排名升降、掉榜名单与连续在榜天数 |
...
工作台「快捷入口」的收藏网址是**刻意的例外**：它只保存在你浏览器的 localStorage 里，**从不写回仓库**。别人 Fork 本项目只会拿到 `src/data/bookmarks.example.json` 这份示例，看不到你的私人链接。代价是换设备时要手动「导出 JSON」再「导入」。

同理，`config/followed.yml` 是**会被提交进仓库**的（因为构建时的 Actions 脚本必须读到它），Fork 时会带上你的关注清单。介意的话可以只保留公开项目，或把它当作可公开的清单使用。

**为什么工作台的书签不放在仓库配置文件里？**
因为「是否进仓库」取决于**谁需要读这份数据**：构建时 Actions 脚本要读的（如 `followed.yml`），只能进仓库；只在浏览器里渲染的（书签），就不该进仓库。仓库里放示例、真实数据留在用户侧，是这个项目对个人数据的默认处理方式。
...
> 版本号以应用内「版本进度面板」的 Changelog 为准，当前最新发布 **v1.16.0**。

| 版本 | 功能 | 状态 |
|------|------|------|
| v1.0 | 核心 MVP：数据采集 + 分类引擎 + 健康评分 + 仪表盘 + GitHub Actions | ✅ |
...
| v1.4 | 版本进度面板（Changelog + Roadmap）+ Trending 实时重算 + 右侧快速导航 | ✅ |
| v1.5 | 搜索 + 高级过滤 + 年度仓库报告（Spotify Wrapped 风格）+ 智能推荐 | ✅ |
| v1.6 | i18n 中英双语 + 仓库笔记/标签 + 数据导出（JSON/CSV）+ 探索下钻 + 变化追踪 | ✅ |
| v1.7 | Tab 化多视图布局（概览 / 仓库 / 动态 / 画像 / 对比） | ✅ |
| v1.8 | 关注项目动态 + 仓库详情侧滑面板 + 变化趋势图 + 贡献热力图 + HTML 报告导出 | ✅ |
| v1.9 | 多用户对比（原 v2.1）+ 学习路径视角（原 v2.8，已于 v1.13 移除） | ✅ |
| v1.10 | 语言分布环形图 + README 速览 + PWA + Release 通知 | ✅ |
| v1.11 | 关注动态卡片化（对齐 GitHub Feed 形态） | ✅ |
| v1.12 | 全屏「查看全部」页 + 侧边快速定位导航 | ✅ |
| v1.13 | 移除学习路径视角 + 暗色图表配色校准 + 构建产物清理 | ✅ |
| v1.14 | GitHub 全局热门榜（附属工具）：今日/本周/本月 × 11 组语言榜 + 排名升降 + 连续在榜 | ✅ |
| v1.15 | 工作台（新 Tab，默认落地页）：待办行动项（待审查 PR / 指派 Issue / 落后上游的 Fork / 沉默仓库）+ 快捷入口书签（本机保存，可导入导出） | ✅ |
| v1.16 | 多用户对比升级：「我和他人」/「他人和他人」两大类，每次最多 4 人，chips 输入，N 侧领先高亮 + 重叠分析（我 vs 每人 / 两两配对） | ✅ |

### 待办

| 项 | 说明 | 状态 |
|----|------|------|
| AI 洞察摘要 | 可选 LLM：画像总结、整理建议、动态周报（原 v2.5） | 🔲 |
| 移动端适配 | 响应式布局优化，手机浏览可用 | 🔲 |
| Fork 价值分析 | 识别哪些 Fork 真正改过代码、哪些可清理 | 🔲 |
| 语言熟练度雷达图 | 按语言数量 / Star / 活跃度给熟练度打分，雷达图展示 | 🔲 |
| 仓库收藏夹 / 置顶 | 标记重要仓库，概览页置顶展示（localStorage） | 🔲 |
| PNG 分享图增强 | 分享卡片支持多主题配色（极简白 / 深色科技 / 渐变） | 🔲 |
| 数据分包加载 | repos.json 约 1MB 内联进主包，可评估按需懒加载 | 🔲 |
